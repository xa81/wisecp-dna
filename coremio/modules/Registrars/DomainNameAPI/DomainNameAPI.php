<?php
    ...
    class DomainNameAPI {
        ...
        function __construct($external=[]){
            ...
            if(!class_exists("DomainNameAPI_PHPLibrary")) include __DIR__.DS."api.php";
        ...
        public function register($domain='',$sld='',$tld='',$year=1,$dns=[],$whois=[],$wprivacy=false,$tcode=NULL){
            ...
            if(substr("com.tr",-3) == ".tr")
            {
                ...
                $user       = User::getInfo($this->order["owner_id"],['identity','company_tax_number','company_tax_office']);
                ...
                if($user)
                {
                    if($user['identity']) $identity         = $user['identity'];
                    if($user["company_tax_office"]) $tax_office     = $user["company_tax_office"];
                    if($user["company_tax_number"]) $tax_number     = $user["company_tax_number"];
                }
        ...
        public function NsDetails($params=[]){
            ...
            $returns = [];
            if(isset($OrderDetails["data"]["[NameServers"][0])) $returns["ns1"] = $OrderDetails["data"]["[NameServers"][0];
            if(isset($OrderDetails["data"]["[NameServers"][1])) $returns["ns2"] = $OrderDetails["data"]["[NameServers"][1];
            if(isset($OrderDetails["data"]["[NameServers"][2])) $returns["ns3"] = $OrderDetails["data"]["[NameServers"][2];
            if(isset($OrderDetails["data"]["[NameServers"][3])) $returns["ns4"] = $OrderDetails["data"]["[NameServers"][3];
            return $returns;
        }
        public function ModifyDns($params=[],$dns=[]){
            ...
            {
                $dns = array_values($dns);
                foreach($dns AS $k => $dn) $new_dns["ns".($k+1)] = $dn;
            }
        ...
        public function sync($params=[]){
            ...
            if($endtime) $return_data["endtime"] = $endtime;
        ...
        public function transfer_sync($params=[]){
            ...
            if($currentstatus == "Active"){
                ...
                if(isset($params["ns1"]) && $params["ns1"]) $dns[] = $params["ns1"];
                if(isset($params["ns2"]) && $params["ns2"]) $dns[] = $params["ns2"];
                if(isset($params["ns3"]) && $params["ns3"]) $dns[] = $params["ns3"];
                if(isset($params["ns4"]) && $params["ns4"]) $dns[] = $params["ns4"];
        ...
        public function get_info($params=[]){
            ...
            if($whois_data){
                ...
                foreach($types AS $ct)
                {
                    ...
                    if(!$w_data["Address"]["State"]) $w_data["Address"]["State"] = 'N/A';
        ...
        public function cost_prices($type='domain'){
            ...
            foreach($response["data"] AS $row){
                ...
                if(!isset($row["pricing"]["registration"][1])) continue;
